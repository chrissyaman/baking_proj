...
const CookiesPage = ({ selectRecipe }) => {
  ...
  return (
    <div>
      <header className="bg-pink-300 py-4 px-3 flex justify-between items-center">
        <div className="text-white text-5xl font-pacifico" style={{ fontFamily: "Pacifico, cursive" }}>
          Epic Baking Website
        </div>
      </header>
      <main className="flex flex-col items-center mt-10">
        <h1 className="text-5xl font-bold mb-10 text-pink-300 font-pacifico" style={{ fontFamily: "Pacifico, cursive" }}>
          Cookies
        </h1>
        <div className="flex flex-wrap justify-center gap-8 px-4 lg:px-0">
          {cookiesRecipes.map((recipe) => (
            ...
            >
              <Link to={`/recipe/${recipe.id}`} className="flex flex-col items-center">
                <img
                  src={recipe.image}
                  alt={recipe.name}
                  className="w-48 h-48 object-cover rounded-md transition-transform transform hover:scale-105"
                />
                <p className="mt-2 text-lg font-medium text-gray-700">{recipe.name}</p>
              </Link>
            </div>
          ))}
        </div>
